...
# del_project_var: Remove a project level environment variable.
#   Note: makes a boolean variable REDEPLOY available to subsequent scripts.
#   $1: Variable name.
#   $2: Target project ID.
del_project_var () {
    unset REDEPLOY
    VAR_STATUS=$(platform project:curl -p $2 /variables/env:$1 | jq '.status')
    if [ "$VAR_STATUS" != "null" ]; then
        echo "Variable $1 does is not defined. Skipping"
        REDEPLOY=false
    else
        echo "Variable $1 found. Deleting."
        platform variable:delete env:$1 -p $2 -l project -y -q
        REDEPLOY=true
    fi
}
...
del_environment () {
    ...
    ENV_CHECK=$(platform project:curl -p $PROJECT /environments/$ENVIRONMENT | jq -r '.status')
    if [ "$ENV_CHECK" = error ]; then
        echo "Environment $ENVIRONMENT does not exist. Skipping"
    else
        platform environment:deactivate -p $1 -e $2 --delete-branch -y
    fi
}
...
cleanup_demo () {
    ...
    for PROJECT in "${PROJECTS[@]}"
    do
        printf "\nCleaning up demo changes on project $PROJECT:\n"
        # Delete environments, starting from the children.
        del_environment $PROJECT new-feature
        del_environment $PROJECT staging
        del_environment $PROJECT auto-updates
        # Remove the project-level variable.
        del_project_var SANITIZE_STAGING $PROJECT
        # Redeploy
        if [ "$REDEPLOY" = true ]; then
            echo "Redeploying affected environments following removing variable."
            # Get the default branch (production environment).
            get_project_default_branch $PROJECT
            # Redeploy the production environment.
            redeploy_env $PROJECT $DEFAULT_BRANCH
        else
            echo "Variable not removed. Skipping redeploy."
        fi
    done
}

# cleanup_demo script-tests